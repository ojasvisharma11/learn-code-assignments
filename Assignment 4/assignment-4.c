...
size_t write_file(int fd,char *data, int length)
{
        int fileResponse = write(fd, data, length);
		return length;
}
...
{
        int fileContent = read(fd, buffer, length);
		return fileContent;
}
...
int main(int argc, char *argv[])
{
    ...
    int size=(int)write_file(fd,buffer, input_data_length);
    if(size<0){
    ...
    size_t write_size=(int)write_file(fd,buffer, input_data_length);

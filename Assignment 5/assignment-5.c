...
student_t* read_student_from_file(FILE *fp)
{
    ...
    if(fp != NULL){
        // printf(OPEN_SUCCESS);
        fscanf(fp, "%d %s %s %s ", &student->id, student->name, student->college, student->passing_year);
        return student;
    }
    else printf(OPEN_ERROR);
    return student;
...
void write_student_to_file(FILE *fp,student_t *student)
{
    ...
    fprintf(fp, "%d %s %s %s ", student->id, student->name, student->college, student->passing_year);
}
...
{
    return fwrite(data, 1, strlen(data), fp);
}
...
int main(int argc, char *argv[])
{
    ...
    if(size!=write_size){
        printf("Invalid Size of the data returned, %d, %ld %s\n", size, write_size, buffer_read);
    }
